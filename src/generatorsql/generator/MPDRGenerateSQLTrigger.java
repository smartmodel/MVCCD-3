...
public abstract class MPDRGenerateSQLTrigger {
    ...
    public String generateSQLDropTrigger(MPDRTrigger mpdrTrigger){
        ...
        generateSQLCode = MPDRGenerateSQLUtil.replaceKeyValue(generateSQLCode, Preferences.MPDR_TRIGGER_NAME_WORD, mpdrTrigger.getName());
    ...
    public String generateSQLCreateTrigger(MPDRTrigger mpdrTrigger) {
        ...
        generateSQLCode = MPDRGenerateSQLUtil.replaceKeyValue(generateSQLCode, Preferences.MPDR_TRIGGER_NAME_WORD, mpdrTrigger.getName());
        generateSQLCode = MPDRGenerateSQLUtil.replaceKeyValue(generateSQLCode, Preferences.MDR_TABLE_NAME_WORD,
        ...
        if (c1 || c2){
            generateSQLCode = MPDRGenerateSQLUtil.replaceKeyValue(generateSQLCode, Preferences.MDR_COLUMN_PK_NAME_WORD,
                    mpdrTrigger.getMPDRTableAccueil().getMPDRColumnPKProper().getName());

            if (c1){
                generateSQLCode = MPDRGenerateSQLUtil.replaceKeyValue(generateSQLCode, Preferences.MPDR_SEQUENCE_NAME_WORD,
