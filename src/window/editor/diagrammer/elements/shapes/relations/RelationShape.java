...
public abstract class RelationShape extends JComponent implements IShape, Serializable {
  ...
  public RelationShape(ClassShape source, SquaredShape destination, IMRelation relatedRepositoryElement,
      boolean isReflexive) {
    this();
    this.source = source;
  ...
  public RelationShape(int id, ClassShape source, SquaredShape destination, boolean isReflexive) {
    this.source = source;
    this.destination = destination;
    this.isReflexive = isReflexive;
    this.id = id;
  ...
  public RelationShape(int id, IMRelation relatedRepositoryElement, ClassShape source,
      SquaredShape destination, boolean isReflexive) {
    this.id = id;
    this.source = source;
    this.destination = destination;
    this.isReflexive = isReflexive;
  ...
  public void drawAnchorPoints(Graphics2D graphics2D) {
    for (RelationAnchorPointShape anchorPoint : this.anchorPoints) {
      graphics2D.fillOval(anchorPoint.x - anchorPoint.DIAMETER / 2,
          anchorPoint.y - anchorPoint.DIAMETER / 2, anchorPoint.DIAMETER, anchorPoint.DIAMETER);
    }
  }
  ...
  public Point getCenter() {
    final RelationAnchorPointShape left = GeometryUtils.getLeftPoint(this.getFirstPoint(),
        this.getLastPoint());
    final RelationAnchorPointShape right = GeometryUtils.getRightPoint(this.getFirstPoint(),
        this.getLastPoint());
    final Line2D segment = new Line2D.Double();
    segment.setLine(this.getFirstPoint().x, this.getFirstPoint().y, this.getLastPoint().x,
        this.getLastPoint().y);
    ...
    if (this.anchorPoints.size() == 2) {
      ...
      return new Point((centerPoint.x + pointNextToCenter.x) / 2,
          (centerPoint.y + pointNextToCenter.y) / 2);
    }
  }
  ...
  public void drawSegments(Graphics2D graphics2D) {
    ...
    for (int i = 0; i < this.anchorPoints.size(); i++) {
      if (i != this.anchorPoints.size() - 1) {
        graphics2D.drawLine((int) this.anchorPoints.get(i).getX(),
  ...
  public void updateFirstAndLastAnchorPoint(ClassShape shape, boolean isResize) {
    ...
    RelationAnchorPointShape point =
        shape == this.source ? this.getFirstPoint() : this.getLastPoint();

    if (isResize) {
      int cursor = shape.getCursor().getType();
      if (cursor == Cursor.E_RESIZE_CURSOR) {
        if (point.y != shape.getBounds().getMaxY() && point.y != shape.getBounds().getMinY()
            && point.x != shape.getBounds().getMinX()) {
        ...
        if (point.y != shape.getBounds().getMaxY() && point.y != shape.getBounds().getMinY()
            && point.x != shape.getBounds().getMaxX()) {
        ...
        if (point.x != shape.getBounds().getMaxX() && point.x != shape.getBounds().getMinX()
            && point.y != shape.getBounds().getMinY()) {
        ...
        if (point.x != shape.getBounds().getMaxX() && point.x != shape.getBounds().getMinX()
            && point.y != shape.getBounds().getMaxY()) {
  ...
  public SquaredShape getNearestClassShape(RelationAnchorPointShape anchorPoint) {
    return anchorPoint == this.getFirstPoint() ? this.source
  ...
  public List<Line2D> getSegments() {
    ...
    for (int i = 0; i < this.anchorPoints.size() - 1; i++) {
      ...
      segment.setLine(this.anchorPoints.get(i).getX(), this.anchorPoints.get(i).getY(),
          this.anchorPoints.get(i + 1).getX(), this.anchorPoints.get(i + 1).getY());
  ...
  public void addAnchorPoint(Point point) {
    ...
    if (nearestSegment != null) {
      int newIndex = this.getAnchorPointIndex(
          this.convertPoint2DToAnchorPoint(nearestSegment.getP2()));
      Point nearestPointOnSegment = GeometryUtils.getNearestPointOnLine(nearestSegment.getX1(),
          nearestSegment.getY1(), nearestSegment.getX2(), nearestSegment.getY2(), point.x, point.y,
          true, null);
      RelationAnchorPointShape newAnchorPoint = new RelationAnchorPointShape(nearestPointOnSegment,
          newIndex);
  ...
  public Line2D getNearestSegment(Point point) {
    for (Line2D segment : this.getSegments()) {
      if (GeometryUtils.getDistanceBetweenLineAndPoint(segment, point)
          <= Preferences.DIAGRAMMER_RELATION_CLICK_AREA) {
        return segment;
      }
    }
    return null;
  ...
  public LabelShape createOrUpdateLabel(RelationAnchorPointShape anchorPoint, String text,
      LabelType type, int distanceInXFromAnchorPoint, int distanceInYFromAnchorPoint) {
    ...
    if (this.hasLabel(type)) {
      ...
      labelShape = this.createLabel(anchorPoint, text, type, distanceInXFromAnchorPoint,
          distanceInYFromAnchorPoint);
    }
  ...
  public LabelShape createLabel(RelationAnchorPointShape anchorPoint, String text, LabelType type,
      int distanceInXFromAnchorPoint, int distanceInYFromAnchorPoint) {

    // La relation possède déjà un label du type fourni en paramètre
    LabelShape label = new LabelShape(anchorPoint, type, this, distanceInXFromAnchorPoint,
        distanceInYFromAnchorPoint);
    label.setText(text);
  ...
  public LabelShape getLabelByType(LabelType type) {
    List<LabelShape> labelsFound = this.labels.stream().filter(l -> l.getType() == type)
  ...
  public void addLabelsInDiagrammeur() {
    ...
    this.labels.forEach(
        label -> DiagrammerService.getDrawPanel().add(label, JLayeredPane.DRAG_LAYER));
  }
  ...
  public void generateAnchorPointWhenDestinationIsRelationShape() {
    ...
    this.anchorPoints.add(
        new RelationAnchorPointShape(this.destination.getCenter().x, this.destination.getCenter().y,
            1));
  ...
  public void generateAnchorPoints() {
    ...
    if (sourcePosition == Position.BOTTOM_CENTER_RIGHT) {
      ...
      this.anchorPoints.add(new RelationAnchorPointShape((int) sourceBounds.getCenterX(),
          (int) sourceBounds.getMaxY(), 0));
      this.anchorPoints.add(new RelationAnchorPointShape((int) sourceBounds.getCenterX(),
          (int) destBounds.getCenterY(), 1));
      this.anchorPoints.add(
          new RelationAnchorPointShape((int) destBounds.getMaxX(), (int) destBounds.getCenterY(),
              2));
      ...
      this.anchorPoints.add(new RelationAnchorPointShape((int) sourceBounds.getCenterX(),
          (int) destBounds.getCenterY(), 1));
      this.anchorPoints.add(
          new RelationAnchorPointShape((int) destBounds.getMinX(), (int) destBounds.getCenterY(),
              2));
    } else if (sourcePosition == Position.BOTTOM_CORNER_RIGHT) {
      this.anchorPoints.add(new RelationAnchorPointShape((int) sourceBounds.getCenterX(),
          (int) sourceBounds.getMinY(), 0));
      this.anchorPoints.add(new RelationAnchorPointShape((int) sourceBounds.getCenterX(),
          (int) destBounds.getCenterY(), 1));
      this.anchorPoints.add(
          new RelationAnchorPointShape((int) destBounds.getMaxX(), (int) destBounds.getCenterY(),
      ...
      if (sourceBounds.intersects(destBounds)) {
        this.anchorPoints.add(new RelationAnchorPointShape((int) sourceBounds.getMaxX(),
            (int) sourceBounds.getCenterY(), 0));
        this.anchorPoints.add(
            new RelationAnchorPointShape((int) destBounds.getMaxX(), (int) destBounds.getCenterY(),
                0));
      }
  ...
  public IMRelation getRelatedRepositoryElement() {
