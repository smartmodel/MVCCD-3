...
public class DrawPanelHandler {
  ...
  public void drawGrid(Graphics2D graphics2D) {
    ...
    for (int i = 0; i < width; i += DrawPanel.getGridSize()) {
      graphics2D.drawLine(i, 0, i, height);
    }
    for (int i = 0; i < height; i += DrawPanel.getGridSize()) {
  ...
  public void moveOrigin(int differenceX, int differenceY) {
    this.drawPanel.getOrigin()
        .translate(GridUtils.alignToGrid(differenceX, DrawPanel.getGridSize()),
            GridUtils.alignToGrid(differenceY, DrawPanel.getGridSize()));
  }

  public void zoomOrigin(int oldGridSize, int newGridSize) {
    this.drawPanel.getOrigin().setLocation(this.drawPanel.getOrigin().x * newGridSize / oldGridSize,
        this.drawPanel.getOrigin().y * newGridSize / oldGridSize);
  }
  ...
  private void removeUnnecessaryWhitespaceAroundDiagram() {
    ...
    for (IShape element : this.drawPanel.getElements()) {
      element
          .setLocation(GridUtils.alignToGrid(element.getBounds().x - newX, DrawPanel.getGridSize()),
              GridUtils.alignToGrid(element.getBounds().y - newY, DrawPanel.getGridSize()));
    }
  ...
  private void checkIfScrollbarsAreNecessary() {
    ...
    if (parent.getHorizontalScrollBar().getValue() < DrawPanel.getGridSize()
        && diaWithoutWhite.getX() + diaWithoutWhite.getWidth()
        <= viewSize.getWidth() + verSbWidth) {
      this.setHorizontalScrollbarVisibility(false);
    } else if (parent.getHorizontalScrollBar().getValue() < DrawPanel.getGridSize()
    ...
    if (parent.getVerticalScrollBar().getValue() < DrawPanel.getGridSize()
        && diaWithoutWhite.getY() + diaWithoutWhite.getHeight()
        <= viewSize.getHeight() + horSbHeight) {
      setVerticalScrollbarVisibility(false);
    } else if (parent.getVerticalScrollBar().getValue() < DrawPanel.getGridSize()
    ...
    if (parent.getHorizontalScrollBar().getValue() != 0 && vertWasVisible
      ...
      adjustedX = GridUtils.alignToGrid(horSbHeight, DrawPanel.getGridSize());
    }
    if (parent.getVerticalScrollBar().getValue() != 0 && horWasVisible
        && !isHorizontalScrollbarVisible()) {
      adjustedY = GridUtils.alignToGrid(verSbWidth, DrawPanel.getGridSize());
    }

    if (adjustedX != 0 || adjustedY != 0) {
      this.drawPanel.setPreferredSize(
          new Dimension((int) (this.drawPanel.getPreferredSize().getWidth() + adjustedX),
              (int) this.drawPanel.getPreferredSize().getHeight() + adjustedY));
  ...
  private void insertWhiteSpaceInUpperLeftCorner() {
    ...
    for (IShape element : this.drawPanel.getElements()) {
      ...
      component.setLocation(
          GridUtils.alignToGrid(component.getX() - adjustWidth, DrawPanel.getGridSize()),
          GridUtils.alignToGrid(component.getY() - adjustHeight, DrawPanel.getGridSize()));
    }
