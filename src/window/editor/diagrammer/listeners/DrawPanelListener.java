...
/*    this.anchorPointsToMove.clear();
  ...
  private void dragDiagram(MouseEvent e) {
    ...
    boolean xNeedsRealignment = this.tempNewX == GridUtils.alignToGrid(this.tempNewX + differenceX,
        DiagrammerService.getDrawPanel().getGridSize());
    boolean yNeedsRealignement = this.tempNewY == GridUtils.alignToGrid(this.tempNewY + differenceY,
        DiagrammerService.getDrawPanel().getGridSize());
  ...
  private void deleteAnchorPointsIfNecessary() {
    ...
    if (this.focusedRelation.getAnchorPoints().size() > 2) {
      ...
      while (iterator.hasNext()) {
        ...
        if (ShapeUtils.getDistanceBetweenLineAndPoint(leftNeighbour, rightNeighbour, pointToCheck)
