...
public class DrawPanelListener extends MouseAdapter implements KeyListener, Serializable {
  ...
  private java.util.List<RelationAnchorPointShape> anchorPointsToMove = new LinkedList<>();


  public DrawPanelListener() {
    this.CURSOR_ENTITY_ICON = Toolkit.getDefaultToolkit().createCustomCursor(
        new ImageIcon("ressources/icons-diagrammer/palette/icon_entity.png").getImage(),
        new Point(0, 0), "cursorEntityIcon");
  }
  ...
  public void mouseMoved(MouseEvent e) {
    ...
    if (this.focusedRelation != null) {
      for (Line2D segment : this.focusedRelation.getSegments()) {
        if (GeometryUtils.getDistanceBetweenLineAndPoint(segment, e.getPoint())
    ...
    if (PalettePanel.activeButton != null && PalettePanel.activeButton.getText()
        .equals(Preferences.DIAGRAMMER_PALETTE_ENTITE_BUTTON_TEXT)) {
  ...
  public void selectAnchorPointsToMove(RelationAnchorPointShape anchorPoint) {
    ...
    if (anchorPoint == this.focusedRelation.getFirstPoint()) {
      // Premier point
      this.anchorPointsToMove.add(
          this.focusedRelation.getAnchorPoints().get(anchorPoint.getIndex() + 1));
  ...
  private void checkForClickedRelation(MouseEvent event) {
    for (RelationShape relation : DiagrammerService.getDrawPanel().getRelationShapes()) {
      for (int i = 0; i < relation.getAnchorPoints().size() - 1; i++) {
        ...
        segment.setLine(relation.getAnchorPoints().get(i).getX(),
            relation.getAnchorPoints().get(i).getY(), relation.getAnchorPoints().get(i + 1).getX(),
            relation.getAnchorPoints().get(i + 1).getY());
  ...
  private void deleteAnchorPointsIfNecessary() {
    ...
    if (this.focusedRelation.getAnchorPoints().size() > 2) {
      final ListIterator<RelationAnchorPointShape> iterator = this.focusedRelation.getAnchorPoints()
          .listIterator();
      ...
      while (iterator.hasNext()) {
        ...
        if (GeometryUtils.getDistanceBetweenLineAndPoint(leftNeighbour, rightNeighbour,
            pointToCheck) < Preferences.DIAGRAMMER_RELATION_CLICK_AREA) {
  ...
  private void dragFirstAnchorPoint(Point newPoint) {
    ...
    boolean dragAllowed = GeometryUtils.pointIsAroundShape(newPoint,
        (SquaredShape) this.focusedRelation.getSource());
  ...
  private void dragLastAnchorPoint(Point newPoint) {
    ...
    if (this.focusedRelation.getDestination() instanceof RelationShape) {
      dragAllowed = GeometryUtils.pointIsOnRelation(newPoint,
          (RelationShape) this.focusedRelation.getDestination());
    } else if (this.focusedRelation.getDestination() instanceof SquaredShape) {
      dragAllowed = GeometryUtils.pointIsAroundShape(newPoint,
          (ClassShape) this.focusedRelation.getDestination());
    }
    if (dragAllowed) {
      this.anchorPointClicked.drag(newPoint.x, newPoint.y);
    }
  ...
  private boolean checkIfAnchorPointCanBeDragged(RelationAnchorPointShape anchorPoint,
      Point newPosition) {
    boolean dragAllowed;

    if (anchorPoint == this.focusedRelation.getFirstPoint()) {
      // Premier point d'ancrage
      dragAllowed = GeometryUtils.pointIsAroundShape(newPosition,
          (SquaredShape) this.focusedRelation.getSource());
    } else if (anchorPoint == this.focusedRelation.getLastPoint()) {
      ...
      if (this.focusedRelation.getDestination() instanceof SquaredShape) {
        ...
        dragAllowed = GeometryUtils.pointIsOnRelation(newPosition,
            (RelationShape) this.focusedRelation.getDestination());
      }
    } else {
  ...
  private void dragAssociation(int differenceX, int differenceY) {
    if (this.focusedRelation.getAnchorPoints().size() == 2) {
      ...
      if (GeometryUtils.isHorizontal(segment)) {
        if (GeometryUtils.pointIsAroundShape(newFirstPoint,
            (SquaredShape) this.focusedRelation.getSource())
            && GeometryUtils.pointIsAroundShape(newSecondPoint,
            (ClassShape) this.focusedRelation.getDestination())) {
  ...
  private void showAnchorPointMenu(MouseEvent event) {
    Point converted = SwingUtilities.convertPoint(this.focusedRelation, event.getPoint(),
  ...
  private void checkForHoveredRelation(MouseEvent e) {
    ...
    for (RelationShape relationShape : DiagrammerService.getDrawPanel().getRelationShapes()) {
      ...
      DiagrammerService.getDrawPanel()
          .setCursor(new Cursor(oneSegmentIsHovered ? Cursor.MOVE_CURSOR : Cursor.DEFAULT_CURSOR));
    }
  }
}