...
public class DrawPanelListener extends MouseAdapter implements KeyListener {
  ...
  public void mouseClicked(MouseEvent e) {
    ...
    if (SwingUtilities.isLeftMouseButton(e)) {
      ...
      System.out.println(this.relationClicked);
      System.out.println(this.pointAncrageClicked);
  ...
  public void mouseDragged(MouseEvent e) {
    ...
    if (this.pointAncrageClicked != null && this.relationClicked != null) {
  ...
  private void executeButtonAction(MouseEvent event) {
    if (PalettePanel.activeButton.getText().equals(
        DiagrammerConstants.DIAGRAMMER_PALETTE_ENTITE_BUTTON_TEXT)) {
      this.createEntityShape(event);
    } else if (PalettePanel.activeButton.getText().equals(
        DiagrammerConstants.DIAGRAMMER_PALETTE_ASSOCIATION_BUTTON_TEXT)) {
  ...
  private void dragPointAncrageSelected(MouseEvent e) {
    ...
    if (pointAncrageClicked.getIndex() == 0 ||
        pointAncrageClicked.getIndex() == relationClicked.getPointsAncrage().getLast().getIndex()) {
  ...
  private void deletePointsAncrageIfNecessary() {
    ...
    if (relationClicked.getPointsAncrage().size() > 2) {
      ListIterator<RelationPointAncrageShape> iterator = relationClicked.getPointsAncrage()
                                                                        .listIterator();
      ...
      while (iterator.hasNext()) {
        ...
        if (GeometryUtils
                .getDistanceBetweenLineAndPoint(leftNeighbour, rightNeighbour, pointToCheck) <
            DiagrammerConstants.DIAGRAMMER_RELATION_CLICK_AREA) {
  ...
  private void dragPointAtIndex1orNMinus1(Point newPoint) {
    ...
    if (pointAncrageClicked.getIndex() == 1 || pointAncrageClicked.getIndex() ==
                                               relationClicked.getPointsAncrage().getLast()
                                                              .getIndex() - 1) {
      // S'il n'y a que 3 points d'ancrage dans l'association
      ClassShape leftShape = (ClassShape) GeometryUtils
                                              .getShapeOnTheLeft(relationClicked.getSource(),
                                                                 relationClicked.getDestination());
      ClassShape rightShape = (ClassShape) GeometryUtils
                                               .getShapeOnTheRight(relationClicked.getSource(),
                                                                   relationClicked
                                                                       .getDestination());
      ...
      if (rightShape.getBounds().getMinY() < leftShape.getBounds().getMinY()) {
        ...
        if (newPoint.y < leftShape.getBounds().getMinY()) {
          ...
          if (previousPoint == relationClicked.getPointsAncrage().getFirst() ||
              previousPoint == relationClicked.getPointsAncrage().getLast()) {
        ...
        if (newPoint.y > rightShape.getBounds().getMaxY()) {
          ...
          if (nextPoint == relationClicked.getPointsAncrage().getLast() ||
              nextPoint == relationClicked.getPointsAncrage().getFirst()) {
        ...
        if ((newPoint.y >= leftShape.getBounds().getMinY() &&
             newPoint.y <= leftShape.getBounds().getMaxY()) &&
            (newPoint.y >= rightShape.getBounds().getMinY() &&
             newPoint.y <= rightShape.getBounds().getMaxY())) {
          newPointRight = new Point((int) rightShape.getBounds().getMinX(), newPoint.y);
          newPointLeft = new Point((int) leftShape.getBounds().getMaxX(), newPoint.y);
        }
        // Si les nouveaux points se situent autour des ClassShape
        if (GeometryUtils.pointIsAroundShape(newPointLeft, leftShape) &&
            GeometryUtils.pointIsAroundShape(newPointRight, rightShape)) {
  ...
  private void showPointAncrageMenu(MouseEvent event) {
    Point converted = SwingUtilities.convertPoint(this.relationClicked, event.getPoint(),
                                                  DiagrammerService.drawPanel);
