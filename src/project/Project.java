...
/**
 * Classe maitresse du projet utilisateur. Divers attributs permettent de mémoriser les
 * caractéristiques du projet utilisateur. Le projet est un élément du référentiel (hérite de
 * MVCCDElement). Les enfants (childs) du projet sont notamment les préférences ainsi que le
 * conteneur de modèles (MCDContModels).
 */
public class Project extends ProjectElement {
  ...
  /**
   * L'id du projet est à indiquer uniquement lors du chargement du projet persisté. Autrement, il
   * faut utiliser le constructeur sans le paramètre "id".
   *
  ...
  /**
   * Le projet n'a pas de parent ! Par contre, il est rattaché à la racine de l'arbre de
   * visualisation du référentiel pour pouvoir le visualiser. Le rattachement se fait par la
   * commande repository.addProject(project) de la méthode projectToRepository() de la classe
   * MVCCDManager.
   *
  ...
  public Profile adjustProfile() {
    if (this.getProfileFileName() != null) {
      ...
      Preferences profilePref = new ProfileLoaderXml().loadFileProfileXML(
  ...
  /**
   * Recherche les préférences du projet en parcourant tous les enfants du projet jusqu'à tomber sur
   * l'objet correspondant aux préférences. Remarque de STB: une alternative est
   * PreferencesManager.getProjectPref()
   *
  ...
  /**
   * Attention, à n'utiliser que pour connaître le dernier numéro d'id qui a été donné par la
   * séquence d'id unique d'éléments d'un projet.
   *
   ...
   *
   * @param id La séquence prendra le numéro passé en paramètre comme dernier numéro déjà attribué à
  ...
  /**
   * Retourne le container de modèles, qui contient le (ou les) MCD qui sont dans le projet de
   * l'utilisateur.
   *
