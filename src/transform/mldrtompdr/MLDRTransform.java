...
public class MLDRTransform extends MDTransform {
  ...
  public boolean transform(MLDRModel mldrModel) {
    ...
    this.mpdrModel = this.foundOrCreateMPDRModel(PreferencesManager.instance().preferences().getMLDRTOMPDR_DB());

    //Clonage du modèle avant transformation
    MPDRModel mpdrModelClone = (MPDRModel) this.mpdrModel.cloneDeep();

    boolean ok = true;
    try {
      this.mpdrModel.incrementeIteration();
      // Au cas où il y aurait changement de préférences
      this.mpdrModel.adjustProperties();

      // Ressources pour les APIs de tables
      if (this.mpdrModel.getMPDR_TAPIs()) {
        if (this.mpdrModel instanceof IMPDRModelRequirePackage) {
          MLDRTransformResources mldrTransformResources = new MLDRTransformResources(this, mldrModel, this.mpdrModel);
          //mldrTransformResources.createOrModifyBoxPackages();
        }
      }

      // Transformation des tables
      MLDRTransformTables mldrTransformTables = new MLDRTransformTables(this, mldrModel, this.mpdrModel);
      mldrTransformTables.transformTables();

      //Etablissement des référencements entre MPDRElement (FKs, Colonnes de FKs...
      this.referencingBetweenPKsAndFKs();

      // Transformation des relationFKs
      MLDRTransformRelations mldrTransformRelations = new MLDRTransformRelations(this, mldrModel, this.mpdrModel);
      ...
      for (MLDRTable mldrTable : mldrModel.getMLDRTables()) {
        ...
        if (mldrTable.getMDRConstraintCustomSpecialized() != null) {
          MLDRTransformToView mldrTransformToView = new MLDRTransformToView(this, (MLDRConstraintCustomSpecialized) mldrTable.getMDRConstraintCustomSpecialized(), this.mpdrModel, this.mpdrModel.getMPDRTableByMLDRTableSource(mldrTable));
      ...
      this.deleteMDRElementNotInIteration();

      //Rafraichir l'arbre
      this.mpdrModel.refreshTreeMPDR();
  ...
  private void undoTransform(MPDRModel mpdrModelClone) {
    ...
    this.mpdrModel.delete();
    mpdrModelClone.setParent((MVCCDElement) this.mldrModel);
  ...
  private MPDRModel foundOrCreateMPDRModel(String mldrtompdrDb) {
    ...
    if (mldrtompdrDb.equals(Preferences.DB_ORACLE)) {
      MPDROracleModel mpdrOracleModel = MLDRModelService.getMPDRModelOracle(this.mldrModel);
      if (mpdrOracleModel == null) {
        mpdrOracleModel = MVCCDElementFactory.instance().createMPDRModelOracle(this.mldrModel);
    ...
    if (mldrtompdrDb.equals(Preferences.DB_MYSQL)) {
      MPDRMySQLModel mpdrMySQLModel = MLDRModelService.getMPDRModelMySQL(this.mldrModel);
      if (mpdrMySQLModel == null) {
        mpdrMySQLModel = MVCCDElementFactory.instance().createMPDRMySQLModel(this.mldrModel);
    ...
    if (mldrtompdrDb.equals(Preferences.DB_POSTGRESQL)) {
      MPDRPostgreSQLModel mpdrPostgreSQLModel = MLDRModelService.getMPDRModelPostgreSQL(this.mldrModel);
      if (mpdrPostgreSQLModel == null) {
        mpdrPostgreSQLModel = MVCCDElementFactory.instance().createMPDRPostgreSQLModel(this.mldrModel);
  ...
  public int getIteration() {
    return this.mpdrModel.getIteration();
  }
  ...
  private void referencingColumnFK(MPDRColumn mpdrColumnFK) {
    ...
    MPDRColumn mpdrColumnPK = (MPDRColumn) this.mpdrModel.getIMPDRElementByMLDRElementSource(mldrColumnPK);
  ...
  private void referencingFK(MPDRFK mpdrFK) {
    ...
    // Complément niveau physique
    MPDRPK mpdrPK = (MPDRPK) this.mpdrModel.getIMPDRElementByMLDRElementSource(mldrPK);
